...
import { EmptyPurchasesMessage, ItemsTitle, PurchaseDetail, PurchaseItem, PurchaseTitle } from './styles';

type Props = {
    purchases: InternalTxn[] | null;
}

const Cashbacks: FC<Props> = ({ purchases }) => {
    return (
        <>
            {purchases?.length === 0 ? (
                <EmptyPurchasesMessage>No cashbacks transactions yet.</EmptyPurchasesMessage>
