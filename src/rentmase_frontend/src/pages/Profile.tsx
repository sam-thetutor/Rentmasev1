    ...
    if (user && user.email) {
      try {
        ...
        await axios(url, {
          ...
          .then((response) => {
            setPohScore(response.data.data)
            console.log("POH Response:", response);
          })
          .catch((error) => {
            console.log("POH Response:", error);
            setPohScore(0);
          });
      } catch (error) {
        setPohScore(0);
    ...
    try {
      ...
      if (pohScore >= 90) {
        ...
        setVerificationResult({
          success: false,
          message:
            "😔 Your POH score is too low. Please complete more quests to increase your score.",
        });
      }
    } catch (error) {
    ...
    if (currentStep === 1) {
      return (
        <>
          ...
          <ModalText>To verify your humanity, you'll need to:</ModalText>
          <ModalText>1. Connect your wallet if you haven't already</ModalText>
          <ModalText>
            2. Ensure you have enough balance for verification
          </ModalText>
          <ModalText>3. Approve the transaction when prompted</ModalText>
        </>
      );
      ...
      return (
        <>
          ...
          <ModalText>Next, we'll need to verify your email:</ModalText>
          <ModalText>1. Check your email for a verification link</ModalText>
          <ModalText>2. Click the link to verify your email</ModalText>
          <ModalText>3. Return to this page once verified</ModalText>
        </>
      );
      ...
      return (
        <>
          ...
          <ModalText>
            Complete the quest to earn extra rewards and increase your POH
            score:
          </ModalText>
          <ModalText>
            1. Visit the quest{" "}
      ...
      return (
        <>
          ...
          <ModalText>Final steps to complete your verification:</ModalText>
          <ModalText>1. Return to your Rentmase profile</ModalText>
          <ModalText>2. Click on "Verify with Intract"</ModalText>
          <ModalText>
          ...
          <ModalButton onClick={handleVerify} disabled={verifying}>
  ...
  return (
    <div>
      <ProfileContainer>
        ...
        {pohScore !== null && (
          ...
          >
            ...
            <POHTooltip visible={showTooltip}>
              Your Proof of Humanity score from Intract. This score reflects
              your verified human status and platform activity.
            </POHTooltip>
          </POHBadge>
        )}
        <ProfileForm>
        ...
        <NavigationButton onClick={() => navigate("/deliveries")}>
          Orders
        </NavigationButton>
        <NavigationButton onClick={() => navigate("/travel-bookings")}>
          Bookings
        </NavigationButton>
        <NavigationButton onClick={() => navigate("/manage-addresses")}>
          Manage Addresses
        </NavigationButton>
        <div className="">
          <WalletBalanceDiv>
          ...
          <h5>Principal : {user?.id.toString()}</h5>
        </div>
      </ProfileContainer>
